...
Ray Camera::generateRay(int x, int y) {
    ...
    */
    Vector3f pixelSample = this->upperLeft + next_float() * this->pixelDeltaU + next_float() * this->pixelDeltaV;
    pixelSample = pixelSample + x * this->pixelDeltaU + y * this->pixelDeltaV;

    Vector3f direction = Normalize(pixelSample - this->from);
